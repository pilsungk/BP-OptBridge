#!/usr/bin/env python3
# bp_lso_pcgrad_tfim_hva_v3.py
#
# LSO-PCGrad experiment on TFIM with gate-local HVA
# (v3: matched-control ablations)
#
# Example:
# python bp_lso_pcgrad_tfim_hva_v3.py --n_qubits 4,6,8,10 --depths 4,6,8 --seeds 30 --steps 100 --lr 0.05
#
# Resume an interrupted run (config must match, schema_version checked):
#   ... --outdir <existing run dir>
#
# Six update rules (see the HEA v3 script for the full description):
#   vanilla, pcgrad (uncapped), pcgrad_nm (norm-matched blind PCGrad),
#   lso_pcgrad (probe-based, 7-8 evaluations), lso_grid (fixed grid,
#   5 evaluations), vanilla_ls_nm (norm-matched vanilla line search,
#   5 evaluations). The matched-budget ablation pair is
#   lso_grid vs vanilla_ls_nm; lso_pcgrad is kept for continuity.
#
...
#                   and pcgrad_nm, 0.0 otherwise.
#   alpha         : variant-specific path parameter (see HEA v3 header).
#   degenerate_anchor : ||u_van|| <= EPS at the current point, recorded
#                   centrally for every variant.
...
RESUME_CONFIG_KEYS = [
    ...
    "variants", "hva_init", "lso_cfg", "h_field",
    "grid_lambdas", "nm_ratio_min",
]
...
def make_timestamped_outdir(root):
    ...
    outdir = os.path.join(root, "bp_lso_pcgrad_tfim_hva_" + ts)
...
def apply_rzz_sv(psi, theta, q1, q2, n):
    dim = 2 ** n
    idx = np.arange(dim, dtype=np.uint64)
    b1 = ((idx >> q1) & 1).astype(np.float64)
    b2 = ((idx >> q2) & 1).astype(np.float64)
    z1 = 1.0 - 2.0 * b1
    z2 = 1.0 - 2.0 * b2
    phase = np.exp(-0.5j * theta * z1 * z2)
    return psi * phase


def initial_state_sv(n, init):
    dim = 2 ** n
    if init == "zero":
    ...
    if init == "plus":
        return np.ones(dim, dtype=np.complex128) / np.sqrt(float(dim))

    if init == "minus":
        idx = np.arange(dim, dtype=np.uint64)
        signs = np.ones(dim, dtype=np.float64)
        for q in range(n):
            signs *= 1.0 - 2.0 * (((idx >> q) & 1).astype(np.float64))
        return signs.astype(np.complex128) / np.sqrt(float(dim))

    raise ValueError("Unknown HVA initial state: " + str(init))
...
def n_params_for_hva(n, depth):
    return (2 * n - 1) * depth


def final_state_sv_hva(params, n, depth, init):
    psi = initial_state_sv(n, init)
    idx = 0

    for _layer in range(depth):
        for q in range(n):
            psi = apply_gate_sv(psi, rx_mat(params[idx]), q, n)
            idx += 1
        for q in range(n - 1):
            psi = apply_rzz_sv(psi, params[idx], q, q + 1, n)
            idx += 1

    return psi


def energy_sv(params, n, depth, H_mat, init):
    psi = final_state_sv_hva(params, n, depth, init)
    e = np.real(np.vdot(psi, H_mat @ psi))
    return float(e)


def grad_termwise_sv(params, n, depth, term_mats, init):
    npar = len(params)
    ...
    for k in range(npar):
        ...
        psi_p = final_state_sv_hva(pp, n, depth, init)
        psi_m = final_state_sv_hva(pm, n, depth, init)
...
def probe_energy_dir(params, n, depth, H_mat, init, upd, lr):
    # Energy at the trial point theta - lr * upd, without committing.
    new_params = wrap_angles(params - lr * upd)
    return energy_sv(new_params, n, depth, H_mat, init)


def probe_energy(params, n, depth, H_mat, init, g_anchor, delta, lr, alpha):
    upd = g_anchor + alpha * delta
    return probe_energy_dir(params, n, depth, H_mat, init, upd, lr)
...
def build_lso_update(params, n, depth, H_mat, init, grad_terms, lr, lso_cfg):
    ...
    f0 = probe_energy(params, n, depth, H_mat, init, g_anchor, delta, lr, a0)
    f1 = probe_energy(params, n, depth, H_mat, init, g_anchor, delta, lr, a1)
    f2 = probe_energy(params, n, depth, H_mat, init, g_anchor, delta, lr, a2)
    probe_calls += 3
    ...
    for alpha in candidate_alphas:
        ...
        if key in seen:
            continue
        seen.add(key)
        val = probe_energy(params, n, depth, H_mat, init, g_anchor, delta, lr, alpha)
        probe_calls += 1
        if val < best_energy:
            best_energy = val
...
def build_pcgrad_nm_update(grad_terms):
    ...
    # aggregate, rescaled to the vanilla gradient norm.
    g_anchor = np.sum(grad_terms, axis=1)
    task_grads = [grad_terms[:, t].copy() for t in range(grad_terms.shape[1])]
    g_pc = pcgrad_project(task_grads)
...
def build_lso_grid_update(params, n, depth, H_mat, init, grad_terms, lr, lso_cfg):
    # LSO on a fixed candidate grid; exactly len(GRID_LAMBDAS) probes.
    # Ties resolved toward the smaller lambda.
    g_anchor = np.sum(grad_terms, axis=1)
    anchor_norm = float(np.linalg.norm(g_anchor))
    ...
    for lam in GRID_LAMBDAS:
        val = probe_energy(params, n, depth, H_mat, init, g_anchor, delta, lr, lam)
...
def build_vanilla_ls_update(params, n, depth, H_mat, init, grad_terms, lr, lso_cfg):
    # Norm-matched vanilla line-search control; each candidate has the
    # same norm as the corresponding lso_grid candidate. Direction is
    # the only difference. Exactly len(GRID_LAMBDAS) probes.
    g_anchor = np.sum(grad_terms, axis=1)
    anchor_norm = float(np.linalg.norm(g_anchor))
    ...
    for lam in GRID_LAMBDAS:
        ...
        val = probe_energy_dir(params, n, depth, H_mat, init, cand, lr)
        energies.append(float(val))
...
def build_update_gradient(params, n, depth, H_mat, init, grad_terms, variant, lr, lso_cfg):
    task_grads = [grad_terms[:, t].copy() for t in range(grad_terms.shape[1])]
    g_anchor = np.sum(grad_terms, axis=1)
    ...
    if variant == "lso_grid":
        return build_lso_grid_update(
            params=params, n=n, depth=depth, H_mat=H_mat, init=init,
            grad_terms=grad_terms, lr=lr, lso_cfg=lso_cfg,
        )
...
def run_single(n, depth, variant, seed, steps, lr, H_mat, term_mats, init, lso_cfg):
    rng = np.random.RandomState(seed)
    npar = n_params_for_hva(n, depth)
    params = rng.uniform(RNG_INIT_LOW, RNG_INIT_HIGH, size=npar).astype(np.float64)

    rows = []

    for step in range(steps + 1):
        e = energy_sv(params, n, depth, H_mat, init)
        grad_terms = grad_termwise_sv(params, n, depth, term_mats, init)
        ...
        # Common v3 metadata, identical schema for every variant. At the
        # final recorded state (step == steps) the update direction is
        # computed but never applied; transition-level analyses must
        # restrict to t = 0..steps-1.
...
def append_trajectory_csv(outdir, rows):
    ...
    print("Appended:", path, "rows=", len(rows))
...
def main():
    parser = argparse.ArgumentParser(
        description="LSO-PCGrad experiment on TFIM termwise HVA."
    )
    ...
    parser.add_argument("--hva_init", type=str, default=DEFAULT_HVA_INIT,
                        choices=["zero", "plus", "minus"])
    ...
    cfg = {
        ...
        "h_field": H_FIELD,
        "ansatz": "TFIM_termwise_HVA",
        "hva_init": init,
        "hva_layer": "RX on each X term, then RZZ on each ZZ edge",
        "n_params": "(2*n_qubits - 1) * depth",
        "variants": variants,
    ...
    for n, depth, variant, seed in product(n_qubits_list, depths, variants, range(seeds)):
        job_key = (n, depth, variant, seed)

        if job_key in completed_jobs:
            done += 1
            print(
                "[{0}/{1}] skip n={2} d={3} variant={4} seed={5}".format(
                    done, total_jobs, n, depth, variant, seed
                )
            )
            continue
